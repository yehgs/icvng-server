//server
// controllers/support-ticket.controller.js  (UPDATED — adds image attachments)
...
export async function getTicketsController(req, res) {
  try {
    ...
    const query = {};
    if (!['IT', 'DIRECTOR'].includes(user.subRole)) query.createdBy = user._id;
    ...
    const tickets = await SupportTicketModel.find(query)
      ...
      .lean();
    return res.json({ success: true, data: tickets, total, page: parseInt(page), totalPages: Math.ceil(total / parseInt(limit)), categories: ISSUE_CATEGORIES });
  } catch (err) {
...
export async function createTicketController(req, res) {
  try {
    ...
    const ticket = await SupportTicketModel.create({
      createdBy: user._id, createdByName: user.name, createdBySubRole: user.subRole, createdByEmail: user.email,
      title, description, category: category || 'Other', priority: priority || 'medium',
      messages: [{ sender: user._id, senderName: user.name, senderSubRole: user.subRole, message: description }],
    });

    await createNotificationInternal({
      triggeredBy: user._id, triggeredByName: user.name, type: 'SUPPORT_TICKET',
      title: `New Support Ticket: ${ticket.ticketNumber}`,
      message: `${user.name} (${user.subRole}) submitted: "${title}" — Category: ${category || 'Other'}`,
      link: `/admin/dashboard/support-tickets`,
      resourceId: ticket._id.toString(), resourceType: 'SupportTicket',
      targetType: 'role', targetRoles: ['IT', 'DIRECTOR'],
...
export async function updateTicketStatusController(req, res) {
  try {
    const user = req.user;
    if (!['IT', 'DIRECTOR'].includes(user.subRole)) return res.status(403).json({ success: false, message: 'Only IT or Director can update status' });
    ...
    if (oldStatus !== status) {
      await createNotificationInternal({
        ...
        link: `/admin/dashboard/support-tickets`,
        resourceId: ticket._id.toString(), resourceType: 'SupportTicket',
        targetType: 'specific', targetUsers: [ticket.createdBy], priority: 'medium',
      });
    }
...
// ─── POST /support-tickets/:id/message — text message ───────────────────────
export async function addTicketMessageController(req, res) {
  try {
    ...
    ticket.messages.push({ sender: user._id, senderName: user.name, senderSubRole: user.subRole, message });
    await ticket.save();

    const notifyRoles = ['IT', 'DIRECTOR'].includes(user.subRole) ? null : ['IT', 'DIRECTOR'];
    const notifyUsers = notifyRoles ? [] : (ticket.assignedTo ? [ticket.assignedTo] : []);

    if (notifyRoles || notifyUsers.length > 0) {
      await createNotificationInternal({
        triggeredBy: user._id, triggeredByName: user.name, type: 'SUPPORT_TICKET',
        ...
        link: `/admin/dashboard/support-tickets`,
        resourceId: ticket._id.toString(), resourceType: 'SupportTicket',
        targetType: notifyRoles ? 'role' : 'specific', targetRoles: notifyRoles, targetUsers: notifyUsers,
        priority: 'medium',
      });
    }
...
// ─── POST /support-tickets/:id/message-image — upload image in thread ────────
export async function addTicketMessageImageController(req, res) {
  try {
    const user = req.user;
    const file = req.file;
    ...
    ticket.messages.push({
      sender: user._id, senderName: user.name, senderSubRole: user.subRole,
      message: `[image] ${file.originalname}`,
      attachments: [{
    ...
    return res.json({ success: true, message: 'Image sent', data: ticket });
  } catch (err) {
    return res.status(500).json({ success: false, message: err.message });
  }
}
